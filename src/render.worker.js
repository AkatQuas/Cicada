...
function renderStroke(s){
  ctx.save();
  if(s.type==='text'){ctx.fillStyle=s.color;ctx.font=`${s.fs}px 'Lora',Georgia,serif`;s.text.split('\n').forEach((ln,i)=>ctx.fillText(ln,s.x,s.y+i*s.fs*1.35));}
  else if(s.type==='circle'){ctx.strokeStyle=s.color;ctx.lineWidth=s.w;ctx.lineCap='round';ctx.beginPath();ctx.arc(s.cx,s.cy,s.r,0,Math.PI*2);ctx.stroke();}
  else{
...
function rdp(pts,eps){
  ...
  let mx=0,mi=0;
  for(let i=1;i<pts.length-1;i++){const d=l2===0?Math.hypot(pts[i].x-a.x,pts[i].y-a.y):Math.abs(dy*pts[i].x-dx*pts[i].y+b.x*a.y-b.y*a.x)/Math.sqrt(l2);if(d>mx){mx=d;mi=i;}}
  return mx>eps?[...rdp(pts.slice(0,mi+1),eps).slice(0,-1),...rdp(pts.slice(mi),eps)]:[a,b];
}

self.onmessage=({data:d})=>{
  if(d.type==='init'){canvas=d.canvas;ctx=canvas.getContext('2d');dpr=d.dpr;vp=d.vp;strokes=d.strokes;redraw();}
  else if(d.type==='update'){if(d.vp)vp=d.vp;if(d.strokes)strokes=d.strokes;if(d.size){canvas.width=d.size.w;canvas.height=d.size.h;}redraw();}
  else if(d.type==='simplify'){
    const eps=d.type==='eraser'?10:([1.5,3,6][PEN_W.indexOf(d.w)]??2);
